...
def drink_window(t = None):
    ...
    end = Button(top, text="Quit Program",command=lambda:quit_program())

    welcome.grid(column=1, row=1)
    label.grid(column=1, row=2)
    create.grid(column=0, row=3)
    end.grid(column=2,row=3)
    admin.grid(column=1, row=3)
...
def log_sale(drink_id, age, user):
    ...
    l["drink_id"] = str(drink_id)
    l["user"] = user["name"]
    l["customer_age"] = str(age)
    l["day"] = str(datetime.now().weekday())
    l["month"] = str(datetime.now().month)
    l["time"] = str(datetime.now().hour)
    
    recommend(str(age), l["day"], l["month"], l["time"])
...
def display_drink_details(data, id):
    if(id!=0):
        ...
        Label(top, image = images[str(id)]).grid(column=0,row=0, rowspan=5)
...
def create_drink(t):
    if(login_check()):
        ...
        # file.grid(column=1)
        fileButton = Button(top,text="Choose File",command=lambda:select_files())
        fileButton.grid(column=1)
        
        submit = Button(top,text="Submit",command=lambda:validate(t, top,name.get(),ingredients.get(),measurements.get(),instructions.get(),description.get()))
        submit.grid(column=1)
    
'''Validates user created drinks'''
def validate(main, t, name:str, ingredients, measurements, instructions, description):
    global temp_drink_file_name
    forbidden_symbols = "~`!@#$%^&*()-_=+][}{\|:;',<.>?/"+'"'
    numbers = "1234567890"
    errors = []
    for l in name:
    ...
    if(len(temp_drink_file_name) > 0):
        try:
            open(temp_drink_file_name)
        except:
    ...
    if(len(errors) != 0):
        ...
        send_drink(main, t, name, create_ingredient_list(ingredients,measurements), instructions, description, temp_drink_file_name)
...
def mock_sales():
    ...
    for i in range(int(num)):
        l = {}
        l["transaction_id"] = str(uuid4())
        l["drink_id"] = str(random.randint(1,len(drink_data)))
        l["user"] = "Mock Data"
        l["customer_age"] = str(random.randint(21,99))
        l["day"] = str(random.randint(0,6))
        l["month"] = str(random.randint(0,11))
        l["time"] = str(random.randint(0,23))
        d.append(l)
...
def select_files():
    ...
    temp_drink_file_name = filename
    
def predict_next_drink(age, day, month, time):
    dbfile = open('model', 'rb')     
    model = pickle.load(dbfile)
    dbfile.close()
    arr = [[age,day,month,time]]
    d = pd.DataFrame(arr,columns = ['customer_age', 'day', 'month', 'time'])
    return model.predict(d)[0]

def recommend(age, day, month, time):
    global window
    top = Toplevel(window)
    top.columnconfigure(0, weight=1)
    top.columnconfigure(1, weight=1)
    top.columnconfigure(2, weight=1)
    top.geometry("300x100")
    top.title("Recommendation")
    next_drink = predict_next_drink(age, day, month, time)
    Label(top, text=drink_data[next_drink-1]["name"]).grid(column=1,row=0)
    e2 = Button(top,text="Okay",command=lambda:top.destroy())
